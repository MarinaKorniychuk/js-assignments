...
function encodeToRot13(str) { 
    let d = {};
    for (let i of [65, 97]) {
        for (let j of Array.from(Array(26).keys())) {
            d[String.fromCharCode(i + j)] = String.fromCharCode(i + (j + 13) % 26);
        }
    }
    return str.split('').map(a => d[a] ? d[a] : a).join('');
}
...
function getCardId(card) {
    ...
    let value = values[card[0]] || +card[0];
    return suits[card[1]] * 13 + value - 1;
}
